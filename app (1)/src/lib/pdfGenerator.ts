...
async function sharePDF(blob: Blob, fileName: string): Promise<void> {
  ...
  if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
    try {
      await navigator.share({
        title: fileName,
        text: `Reporte VoeseSystem - ${fileName}`,
        files: [file]
      });
    } catch (error) {
...
export async function generatePDF(data: PDFData): Promise<void> {
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('EFECTIVO EN CAJA', margin + 5, yPos + 10);
  doc.setTextColor(textColor);
  doc.setFontSize(16);
  doc.text(`₲ ${formatMoney(data.efectivoTotal)}`, margin + 5, yPos + 25);

  doc.setFillColor(lightGray);
  doc.roundedRect(margin + 90, yPos, 85, 35, 3, 3, 'F');
  doc.setTextColor(primaryColor);
  doc.setFontSize(10);
  doc.text('VENTAS DEL MES', margin + 95, yPos + 10);
  doc.setTextColor(textColor);
  doc.setFontSize(16);
...
export async function generateYearlyPDF(
  ...
  doc.setTextColor(primaryColor);
  doc.setFontSize(10);
  doc.text('EFECTIVO EN CAJA', margin + 95, yPos + 8);
  doc.setTextColor(textColor);
  doc.setFontSize(14);
  doc.text(`₲ ${formatMoney(efectivoTotal)}`, margin + 95, yPos + 22);
  ...
  // Generar blob y compartir
  const fileName = `VoeseSystem_${year}_Anual.pdf`;
  const pdfBlob = doc.output('blob');
  await sharePDF(pdfBlob, fileName);
}
